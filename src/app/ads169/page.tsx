...
export default function ADS169Page() {
  ...
  useEffect(() => {
    setMounted(true)
    const savedLeftOpen = localStorage.getItem('ads169-leftOpen')
    const savedRightOpen = localStorage.getItem('ads169-rightOpen')
  ...
  useEffect(() => {
    ...
    if (tabParam) {
      ...
      if (foundTab) {
        ...
        localStorage.setItem('ads169-activeTab', String(foundTab.id))
      }
    } else {
      // If no URL param, check localStorage and update URL
      const savedTab = localStorage.getItem('ads169-activeTab')
      if (savedTab) {
        const tabId = parseInt(savedTab)
        setActiveTab(tabId)
        const tab = tabs.find(t => t.id === tabId)
        if (tab) {
          router.replace(`/ads169?tab=${tab.slug}`, { scroll: false })
        }
      } else {
        // Default to first tab
        router.replace(`/ads169?tab=${tabs[0].slug}`, { scroll: false })
      }
    }
  ...
  const handleLeftToggle = (newState: boolean) => {
    ...
    localStorage.setItem('ads169-leftOpen', String(newState))
  }

  // Save right panel state to localStorage
  const handleRightToggle = (newState: boolean) => {
    setIsRightOpen(newState)
    localStorage.setItem('ads169-rightOpen', String(newState))
  }

  // Update URL when tab changes and save to localStorage
  const handleTabChange = (tabId: number) => {
    setActiveTab(tabId)
    localStorage.setItem('ads169-activeTab', String(tabId))
    const tab = tabs.find(t => t.id === tabId)
    if (tab) {
      router.push(`/ads169?tab=${tab.slug}`, { scroll: false })
    }
  }
    ...
    <div className="h-full flex gap-4 relative">
      ...
      <Card className={cn(
        ...
        <CardContent className="flex-1 p-6 overflow-auto">
          {activeTab === 1 && (
            <div>
              <h2 className="text-2xl font-bold mb-4">ADS169 - แท็ป 1</h2>
          ...
          {activeTab === 2 && (
            <div>
              <h2 className="text-2xl font-bold mb-4">ADS169 - แท็ป 2</h2>
              <div className="border border-white/10 rounded-xl overflow-hidden bg-white/5 dark:bg-black/5 backdrop-blur-2xl shadow-2xl">
                <Table>
                  <TableHeader>
                    <TableRow className="border-white/10 hover:bg-white/5 dark:hover:bg-white/5 bg-white/10 dark:bg-black/10 backdrop-blur-xl">
          ...
          {activeTab === 3 && (
            <div>
              <h2 className="text-2xl font-bold mb-4">ADS169 - แท็ป 3</h2>
              <div className="border border-white/10 rounded-xl overflow-hidden bg-white/5 dark:bg-black/5 backdrop-blur-2xl shadow-2xl">
                <Table>
                  <TableHeader>
                    <TableRow className="border-white/10 hover:bg-white/5 dark:hover:bg-white/5 bg-white/10 dark:bg-black/10 backdrop-blur-xl">
